...
var summerHtmlImageMapCreator = (function() {
    ...
    var app = (function() {
        ...
        function onAreaDblClick(e) {
            if (state.appMode === 'editing') {
                console.log('function onAreaDblClick()');
            ...
            return {
                save : function() {
                   ...
                   // console.info('Editor ' + result + ' saved');
                    ...
                    $('body').pgNotification({style:'flip',message:'Plan stoisk został zapisany',position:'top-right',timeout:2000,type:'success'}).show();
                },
                restore : function() {
    ...
    Area.REGEXP = {
        ...
        STATION_NAME : / station_name="([\S\s]+?)"/,
        STATION_PRICE : / station_price="([\S\s\d]+?)"/,
        STATION_AREA : / station_area="([\S\s\d]+?)"/,
        STATION_KEY : / station_key="([\S\s\d]+?)"/,
        ACTIVE : / active="([\d]+?)"/,
        DELIMETER : / ?, ?/
    };
    ...
    Area.ATTRIBUTES_NAMES = ['STATION_NAME', 'STATION_PRICE', 'STATION_AREA', 'STATION_KEY' , 'ACTIVE'];
     ...
     * @returns {Area} - this area
     */
    Area.prototype.select = function() { console.log(this._attributes);
        if(this._attributes.active != 1 && this._attributes.active != '') {
            $('body').pgNotification({style:'flip',message:'Stanowisko zarezerwowane. Edycja zablokowana.',position:'top-right',timeout:4000,type:'error'}).show();
            return false;
        }
    ...
    Area.prototype.setInfoAttributes = function(attributes) {
        this._attributes.station_name = attributes.station_name;
        this._attributes.station_price = attributes.station_price;
        this._attributes.station_area = attributes.station_area;
        this._attributes.station_key = attributes.station_key;
        this._attributes.active = attributes.active;
    };
    ...
    Area.copy = function(originalArea) {
        var copiedObject = jQuery.extend(true, {}, originalArea)
        var original = copiedObject.toJSON();
        original.attributes.station_key = '';
        original.attributes.active = 1;

        return Area.fromJSON(original).move(10, 10).select();
    };
    ...
    Rectangle.prototype.toHTMLMapElementString = function() {
        ...
        return '<area shape="rect" coords="' // TODO: use template engine
            ...
            + y2
            + '"'
            + (this._attributes.station_name ? ' station_name="' + this._attributes.station_name + '"' : '')
            + (this._attributes.station_price ? ' station_price="' + this._attributes.station_price + '"' : '')
            + (this._attributes.station_area ? ' station_area="' + this._attributes.station_area + '"' : '')
            + (this._attributes.station_key ? ' station_key="' + this._attributes.station_key + '"' : '')
            + ' />';
    };
    ...
    var info = (function() {
        var form = utils.id('edit_details_div'),
            header = form.querySelector('h5'),
            station_name = utils.id('station_name'),
            station_price = utils.id('station_price'),
            station_area = utils.id('station_area'),
            station_key = utils.id('station_key'),
            save_button = utils.id('save_details'),
            close_button = form.querySelector('.close_button'),
            sections = form.querySelectorAll('.section_edit'),
            obj,
            x,
            y,
            temp_x,
        ...
        function save(e) {
            ...
            obj.setInfoAttributes({
                ...
                station_price : station_price.value,
                station_area : station_area.value,
                station_key : (station_key.value == '' ? (Date.now().toString(36) + Math.random().toString(36).substr(2, 10)) : station_key.value)
            });

            console.log(obj._attributes);
            
            obj[obj._attributes.station_name ? 'setStyleOfElementWithHref' : 'unsetStyleOfElementWithHref']();
...
//            utils.hide(form);
        ...
        station_name.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        station_price.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);
        station_area.addEventListener('keydown', function(e) { e.stopPropagation(); }, false);

        station_name.addEventListener('change', change, false);
        station_price.addEventListener('change', change, false);
        station_area.addEventListener('change', change, false);
        ...
        return {
            load : function(object, new_x, new_y) {
                obj = object;
                station_name.value = object._attributes.station_name ? object._attributes.station_name : '';
                station_price.value = object._attributes.station_price ? object._attributes.station_price : '';
                station_area.value = object._attributes.station_area ? object._attributes.station_area : '';
                station_key.value = object._attributes.station_key ? object._attributes.station_key : '';
                $('#edit_details_div').modal('show');
                //utils.show(form);
                if (new_x && new_y) {
                    x = 0;
                    y = 70;
    ...
    app.loadImage(app.domElements.img.src);

    var mapHtml = utils.id('mapHtml')
    Area.createAreasFromHTMLOfMap(JSON.parse(mapHtml.value));


    /* Buttons and actions */
    var buttons = (function() {
        var all = utils.id('nav').getElementsByTagName('button'),
            save = utils.id('save'),
            load = utils.id('load'),
        ...
        function deselectAll() {
            utils.foreach(all, function(x) {
                ...
                x.classList.remove(Area.CLASS_NAMES.ACTIVE);
            });
        }
