...
abstract class Model implements ModelInterface {
    ...
    public function getPair($filtr = array(), $sort = null, $limit = null, $val = '')
    {
        $result = $this->getAll($filtr, $sort, $limit);

        $newArray = array();
        foreach ($result as $row) {
            $newArray[$row[$this->getFirstPrimaryKey()]] = $row[$val];
        }
    ...
    public function updateWhere($data, $where)
    {
        ...
        if (count($dataToDb) > 0) {
            $affected = $this->dbTable->update($dataToDb, $where);
        }
