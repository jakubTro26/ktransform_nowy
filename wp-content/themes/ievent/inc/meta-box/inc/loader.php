...
class RWMB_Loader {
	...
	protected function constants() {
		...
		define( 'ievent_RWMB_VER', '4.9.8' );

		list( $path, $url ) = self::get_path( dirname( dirname( __FILE__ ) ) );

		// Plugin URLs, for fast enqueuing scripts and styles
		define( 'ievent_RWMB_URL', $url );
		define( 'ievent_RWMB_JS_URL', trailingslashit( ievent_RWMB_URL . 'js' ) );
		define( 'ievent_RWMB_CSS_URL', trailingslashit( ievent_RWMB_URL . 'css' ) );

		// Plugin paths, for including files
		define( 'ievent_RWMB_DIR', $path );
		define( 'ievent_RWMB_INC_DIR', trailingslashit( ievent_RWMB_DIR . 'inc' ) );
	}
	...
	public function init() {
		...
		require_once ievent_RWMB_INC_DIR . 'autoloader.php';
		$autoloader = new RWMB_Autoloader;
		$autoloader->add( ievent_RWMB_INC_DIR, 'RW_' );
		$autoloader->add( ievent_RWMB_INC_DIR, 'RWMB_' );
		$autoloader->add( ievent_RWMB_INC_DIR . 'fields', 'RWMB_', '_Field' );
		$autoloader->add( ievent_RWMB_INC_DIR . 'walkers', 'RWMB_Walker_' );
		$autoloader->register();
		...
		// Public functions
		require_once ievent_RWMB_INC_DIR . 'functions.php';
	}
}
