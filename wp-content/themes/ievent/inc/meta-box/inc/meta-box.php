...
class RW_Meta_Box {
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'rwmb', ievent_RWMB_CSS_URL . 'style.css', array(), ievent_RWMB_VER );
		if ( is_rtl() ) {
			wp_enqueue_style( 'rwmb-rtl', ievent_RWMB_CSS_URL . 'style-rtl.css', array(), ievent_RWMB_VER );
		}

		// Load clone script conditionally
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'rwmb-clone', ievent_RWMB_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), ievent_RWMB_VER, true );
				break;
			}
		}
		...
		if ( $this->meta_box['autosave'] ) {
			wp_enqueue_script( 'rwmb-autosave', ievent_RWMB_JS_URL . 'autosave.js', array( 'jquery' ), ievent_RWMB_VER, true );
		}
